...
function drawActions() {
  ...
  container.innerHTML = actions.length ? actions.map((action, index) => `
    <article class="action-card ${action.needs_review ? 'review-needed' : ''}" data-action-row="${index}" data-status="${action.status || 'draft'}" data-review="${action.needs_review}" data-search="${escapeAttribute(`${action.task} ${action.responsible}`.toLocaleLowerCase('ru'))}">
      ...
      <label class="action-field status-field"><span>Статус</span><select data-index="${index}" data-key="status"><option value="draft" ${!action.status || action.status === 'draft' ? 'selected' : ''}>Черновик</option><option value="in_progress" ${action.status === 'in_progress' ? 'selected' : ''}>В работе</option><option value="overdue" ${action.status === 'overdue' ? 'selected' : ''}>Просрочено</option><option value="done" ${action.status === 'done' ? 'selected' : ''}>Выполнено</option></select></label>
