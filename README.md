...
Основные API endpoints: `POST /process`, `POST /demo/{id}`, `GET /dashboard`, `GET /reports`, `GET /reports/{id}`, `PUT /reports/{id}` и `GET /download/{id}/{kind}`. PUT принимает версию `revision`, только проверяемые поля и сохраняет исходный evidence и транскрипт.
